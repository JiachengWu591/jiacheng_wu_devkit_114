...
devkit 的所有功能都挂在三个子命令组下面:``convert``、``batch``、``log``。下面按组逐一介绍每个子命令的作用、参数和示例。
...
``devkit convert pdf2md`` 用于把 PDF 转成 Markdown 文本:
...
- ``--dry-run``:只打印重命名计划就退出,不会改动任何文件。注意:这一步**不会**执行下面说的冲突检查,只是把计划打出来给你看,所以在 ``--dry-run`` 下看起来没问题的计划,真正执行时仍可能因为冲突被拒绝。
...
- ``--dry-run``:只打印移动计划就退出,不会改动任何文件(同样,这一步也不会执行冲突检查)。
- ``--yes``/``-y``:跳过交互式确认,直接执行。
...
- **batch 的安全机制分三步,``--dry-run`` 只覆盖第一步。** ``batch`` 的两个子命令(``rename``、``organize``)在改动任何文件之前会依次做三件事:第一步,把完整的"旧路径 -> 新路径"迁移计划打印出来(用的是每个文件的完整绝对路径);第二步,对这份计划做冲突检查——如果发现两个文件会被移到同一个目标位置,或者某个目标路径已经存在于磁盘上且并非本次计划自己要移动的源文件,命令会直接拒绝执行,不做任何改动(哪怕只有一处冲突,也不会部分执行);第三步,只要没有加 ``--dry-run`` 或 ``--yes``/``-y``,命令会用交互式提示(类似 "Rename N file(s)?" / "Move N file(s)?")等你确认,只有回答"是"才会真正动手。**关键点在于:第二步的冲突检查只有在不加 ``--dry-run`` 时才会执行。**``--dry-run`` 只负责打印计划,并不会替你顺带做冲突检查,所以在 ``--dry-run`` 下看起来"干净"的计划,真正执行时仍有可能因为冲突被拒绝。好消息是:只要不加 ``--dry-run`` (哪怕同时加了 ``--yes``),冲突检查一定会在改动任何文件之前先跑一遍——一旦发现冲突就直接拒绝退出,不会有任何文件被碰到,所以正式执行本身依然是安全的。
